"""
═══════════════════════════════════════════════════════════════
PRUEBA 5.1 — Misión cooperativa completa (SLAVE, Ubuntu)
═══════════════════════════════════════════════════════════════
...
def main():
    ...
    logger = FlightLogger("test_5_1_slave")
    ...
    try:
        while True:
            ...
            # 6 = master landing, 7 = emergency, 8 = master ordena al slave aterrizar (5.1)
            if master_msg and master_msg.mission_state in (6, 7, 8):
                ms = master_msg.mission_state
                reason = {6: "master landing", 7: "emergency",
                          8: "MASTER ORDENA SLAVE ATERRIZAR"}[ms]
                print(f"[INFO] MASTER notificó: {reason} "
                      f"(mission_state={ms}). Aterrizando.")
