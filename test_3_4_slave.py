"""
═══════════════════════════════════════════════════════════════
PRUEBA 3.4 — Tolerancia a fallas de comunicación (SLAVE, Ubuntu)
═══════════════════════════════════════════════════════════════
...
def main():
    ...
    logger = FlightLogger("test_3_4_slave")
    ...
    print(f"[INFO] DESCONECTA EL CABLE ETHERNET cuando quieras evaluar la pérdida.")
    print(f"[INFO] Te recomiendo: 5 s, esperar recovery, 10 s, esperar, 15 s.\n")
    t_start = time.time()
    last_pose_t = t_start
    errors_3d = []
    formation_errors = []
    ...
    # ----- Métricas de la 3.4: detectar eventos basados en master_age -----
    # (más robusto que basarse en feedback string, que puede oscilar entre
    # "lost_master" y "lost_self" cuando además el SLAVE pierde su pose)
    is_master_lost = False    # estado actual del enlace
    disconnect_events = []
    pos_at_disconnect = None
    pos_during_disconnect_max_disp = 0.0
    last_disc_t = None
    reconverge_target_t = None
    ...
    try:
        while True:
            ...
            # ----- 3.4: detectar eventos basados directamente en master_age -----
            # IMPORTANTE: siempre poblar todas las claves (None si no aplica)
            # para que el CSV tenga schema fijo desde la primera fila.
            row["disconnect_event"] = None
            row["disp_during_disconnect_cm"] = None
            curr_master_lost = (master_age > LOST_TIMEOUT_S)

            # Transición conectado → desconectado
            if (not is_master_lost) and curr_master_lost:
                last_disc_t = t_now
                pos_at_disconnect = pos.copy() if pos else None
                pos_during_disconnect_max_disp = 0.0
                is_master_lost = True
                row["disconnect_event"] = "lost"
                print(f"\n[3.4] ⚠️  ENLACE PERDIDO en t={t_now-t_start:.1f}s. "
                      f"SLAVE en hover seguro.")

            # Durante desconexión: trackear desplazamiento
            if is_master_lost and pos and pos_at_disconnect:
                disp = math.sqrt(
                    (pos["x"] - pos_at_disconnect["x"])**2 +
                    (pos["y"] - pos_at_disconnect["y"])**2 +
                    (pos["z"] - pos_at_disconnect["z"])**2)
                if disp > pos_during_disconnect_max_disp:
                    pos_during_disconnect_max_disp = disp
                row["disp_during_disconnect_cm"] = disp * 100

            # Transición desconectado → conectado
            if is_master_lost and not curr_master_lost:
                duration = t_now - last_disc_t
                ev = {
                    "t_lost_s": last_disc_t - t_start,
                    "duration_s": duration,
                    "max_disp_cm": pos_during_disconnect_max_disp * 100,
                    "recov_s": None,
                    "_recov_start_t": t_now,   # interno
                }
                disconnect_events.append(ev)
                is_master_lost = False
                reconverge_target_t = None
                row["disconnect_event"] = "back"
                print(f"[3.4] ✓ ENLACE RECUPERADO en t={t_now-t_start:.1f}s. "
                      f"Duración: {duration:.1f}s, máx desplazamiento: "
                      f"{ev['max_disp_cm']:.1f}cm. Esperando reconvergencia...")

            # Después de reconectar: medir cuánto tarda en reconverger
            if (not is_master_lost) and disconnect_events and disconnect_events[-1]["recov_s"] is None:
                ev = disconnect_events[-1]
                if row.get("err_3d_cm") is not None and row["err_3d_cm"] < 15.0:
                    if reconverge_target_t is None:
                        reconverge_target_t = t_now
                    elif t_now - reconverge_target_t >= 1.0:
                        ev["recov_s"] = reconverge_target_t - ev["_recov_start_t"]
                        del ev["_recov_start_t"]
                        print(f"[3.4] ✓✓ RECONVERGIDO en {ev['recov_s']:.2f}s.\n")
                        reconverge_target_t = None
                else:
                    reconverge_target_t = None
        ...
        if errors_3d and formation_errors:
            ...
            print("\n" + "="*64)
            print("RESULTADOS — SLAVE (Tello B), Prueba 3.4 Tolerancia a fallas")
            print("="*64)
        ...
        # ----- Resumen específico 3.4: eventos de desconexión -----
        print("\n" + "="*64)
        print("EVENTOS DE DESCONEXIÓN / RECONEXIÓN — Prueba 3.4")
        print("="*64)
        if not disconnect_events:
            print("  No se detectaron desconexiones completas.")
            if is_master_lost and last_disc_t is not None:
                print(f"  Hubo desconexión activa al terminar el vuelo "
                      f"(t_lost={last_disc_t-t_start:.1f}s, "
                      f"máx desplazamiento durante: "
                      f"{pos_during_disconnect_max_disp*100:.1f} cm, sin reconectar).")
        else:
            print(f"  {'#':>2} {'t_lost':>8} {'duración':>10} {'max_disp':>10} {'recov':>8}")
            print(f"  {'-'*2:>2} {'-'*8:>8} {'-'*10:>10} {'-'*10:>10} {'-'*8:>8}")
            for i, ev in enumerate(disconnect_events, 1):
                dur = ev.get("duration_s", 0)
                disp = ev.get("max_disp_cm", 0)
                recov = ev.get("recov_s", None)
                recov_str = f"{recov:.2f}s" if recov is not None else "no_recov"
                print(f"  {i:>2} {ev['t_lost_s']:>8.1f}s {dur:>9.1f}s {disp:>9.1f}cm {recov_str:>8}")
        print("="*64)
        print("[DONE] Prueba 3.4 SLAVE completada.")
